...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    const { name, studentId, programstudi, campuses, contactinfo, pelanggarans, buktiinfo, deskripsiinfo, image } = req.body;
    ...
    try {
      ...
      const result = await collection.insertOne({ name, studentId, programstudi, campuses, contactinfo, pelanggarans, buktiinfo, deskripsiinfo, image, createdAt: new Date() });
